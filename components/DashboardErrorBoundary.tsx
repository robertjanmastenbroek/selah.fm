        ...
        <div className="min-h-screen flex items-center justify-center px-4" style={{ background: '#0F0F23' }}>
          <div className="text-center max-w-2xl space-y-4">
            ...
            <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-4 text-left">
              <p className="text-sm font-mono text-red-300 break-all whitespace-pre-wrap">Message: {this.state.error.message}</p>
              {this.state.error.stack && (
                <details className="mt-2">
                  <summary className="text-xs text-muted-foreground cursor-pointer">Stack trace</summary>
                  <pre className="text-[9px] text-muted-foreground/50 mt-1 whitespace-pre-wrap max-h-96 overflow-y-auto">
                    {this.state.error.stack}
                  </pre>
                </details>
              )}
