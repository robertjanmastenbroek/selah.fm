...
export default function RootPage({ 
  ...
  useEffect(() => {
    ...
    fetch('/api/campaigns?limit=6&sort=recent').then(r => r.json()).then(d => {
      if (d.campaigns) setFeaturedCampaigns(d.campaigns);
      if (d.total) setTotalActive(d.total);
    }).catch(() => {});
  }, []);
      ...
      <section className="relative z-10 px-4 py-32 max-w-2xl mx-auto">
        ...
        <div className="space-y-2.5">
          {[
            ...
            { q: 'How much does it cost?', a: 'Nothing upfront. You set a CPM rate (e.g. $10 per 1,000 views) and a budget. Creators earn 80%, we take 20%. Stripe fees are separate.' },
          ].map((faq, i) => (
