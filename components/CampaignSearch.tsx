    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        <input type="text" value={query} onChange={(e) => { setQuery(e.target.value); handleChange(); }}
          ...
          className="flex-1 bg-void-card border border-white/10 rounded-xl px-4 py-3 text-ivory text-sm
        ...
        {PLATFORMS.map((p) => (
          <button key={p} onClick={() => { setPlatform(p); handleChange(); }}
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all
              ${platform === p ? 'bg-gold/20 text-gold border border-gold/30' : 'bg-void-card text-muted border border-white/5 hover:border-white/10'}`}>
            {p === 'all' ? 'All' : p}
          </button>
        ))}
        <select value={minCPM} onChange={(e) => { setMinCPM(Number(e.target.value)); handleChange(); }}
          className="bg-void-card border border-white/5 rounded-lg px-2 py-1.5 text-xs text-muted focus:outline-none focus:border-gold/30">
