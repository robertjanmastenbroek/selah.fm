...
export default function ArtistCard({ artist, actionLoading, onAudit, onCreateCampaign, onSendEmail, onRenderOutreach, onRenderFollowUp, onLogOutreach, onSkip }: Props) {
  ...
  const isBusy = actionLoading.startsWith('audit-') || actionLoading.startsWith('campaign-') || actionLoading.startsWith('email-') || actionLoading.startsWith('outreach-') || actionLoading.startsWith('followup-') || actionLoading.startsWith('skip-') || actionLoading === 'discover' || actionLoading === 'batch-audit';
    ...
    <div className="rounded-2xl bg-white/[0.02] border border-white/[0.05] p-4 hover:bg-white/[0.04] transition-colors">
      <div className="flex items-start gap-4">
        {artist.latest_track_cover_url ? (
          <img src={artist.latest_track_cover_url} alt="" className="w-12 h-12 rounded-xl object-cover shrink-0" loading="lazy" />
        ) : (
          <div className="w-12 h-12 rounded-xl shrink-0 bg-white/[0.04] flex items-center justify-center">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium ${s.bg} ${s.color}`}>{s.label}</span>
            {artist.status === 'audited' && !hasEmail && (
              <span className="shrink-0 px-2 py-0.5 rounded-full text-[10px] font-medium bg-red-500/10 text-red-400">No email</span>
            )}
          </div>
