              ...
              {earnings.submissions.length === 0 ? (
                ...
                earnings.submissions.map((s: Submission, i: number) => (
                  <div key={s.id}>
                    ...
                    {/* Share + Rating: show if paid */}
                    {s.payout_status === 'paid' && (
                      <div className="mt-2 mb-3 space-y-2">
                        <button
                          onClick={() => {
                            const amount = formatMoney(s.payout_amount_cents);
                            const platform = s.platform.charAt(0).toUpperCase() + s.platform.slice(1);
                            const text = `Just got paid ${amount} for making a ${platform} video on @selahfm! Creators earn per verified view.\n\nselah.fm`;
                            window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}`, '_blank', 'noopener,noreferrer');
                          }}
                          className="w-full py-2 rounded-lg bg-[#1DA1F2]/10 hover:bg-[#1DA1F2]/20 border border-[#1DA1F2]/20 text-[#1DA1F2] text-xs font-semibold transition-colors flex items-center justify-center gap-1.5"
                        >
                          <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg> Share on X
                        </button>
                        <RatingPrompt
                          submissionId={s.id}
                          role="creator"
