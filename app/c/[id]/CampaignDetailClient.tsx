...
export default function CampaignDetailClient({ id, initialCampaign }: { id: string; initialCampaign: any }) {
  ...
  const handleDonate = async () => { try { const r = await fetch(`/api/campaigns/${id}/support`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount: donationAmount }) }); const d = await r.json(); if (d.clientSecret) { setClientSecret(d.clientSecret); setPaymentModal(true); } else addToast(d.error || 'Error', 'error'); } catch { addToast('Network error', 'error'); } };
