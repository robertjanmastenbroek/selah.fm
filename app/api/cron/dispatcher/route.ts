 ...
 *
 * ─── DISABLED 2026-06-24 to stop egress burn while project is paused.
 *     All cron workers are commented out below. The dispatcher still runs
 *     hourly (via railway.json cron entry) but does nothing.
 *     To re-enable: uncomment the WORKERS map and the loop body below.
 *     The railway.json cron entry can stay disabled until you're ready.
 */

// ponytail: all cron work paused — egress burn investigation
...
//   11: [{ path: '/api/cron/creator-outreach' }, { path: '/api/cron/reengage' }, { path: '/api/cron/submit-to-google' }],
//   12: [{ path: '/api/cron/message-notifications' }, { path: '/api/cron/finalize-payouts' }],
