      ...
      <div className="w-full max-w-sm space-y-4">
        ...
        {forgotMode ? (
          <form onSubmit={handleForgot} className="space-y-4">
            <h2 className="text-lg font-bold text-center">{t('resetPassword')}</h2>
            <p className="text-sm text-muted-foreground text-center">{t('enterEmailAndSendReset')}</p>
            {forgotSent ? (
              <p className="text-sm text-emerald-400 text-center">{t('checkEmailForResetLink')}</p>
            ) : (
              <>
                ...
                <Button type="submit" disabled={loading} className="w-full h-12 rounded-xl font-semibold">{loading ? t('sending') : t('sendResetLink')}</Button>
              </>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {mode === 'signup' && (
                <div className="grid grid-cols-3 gap-2">
                  {[{r:'artist',label:t('roleArtist'),desc:t('roleArtistDesc')},{r:'creator',label:t('roleCreator'),desc:t('roleCreatorDesc')},{r:'fan',label:t('roleFan'),desc:t('roleFanDesc')}].map(({r,label,desc})=>(
              ...
              {mode === 'signup' && (
                <Input type="text" value={name} onChange={e => setName(e.target.value)} placeholder={t('displayName')}
              ...
              <Input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('emailAddress')}
                className="h-12 rounded-xl bg-white/[0.04] border-white/[0.06]" />
              <Input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder={t('passwordMinChars')}
                className="h-12 rounded-xl bg-white/[0.04] border-white/[0.06]" />
              ...
              {mode === 'signup' && (
                <label className="flex items-start gap-2 cursor-pointer">
                  ...
                  <span className="text-[11px] text-muted-foreground leading-relaxed">
                    {t('iAm13Plus')}{' '}
                    <a href="/tos" className="text-primary hover:underline" target="_blank">{t('terms')}</a> and{' '}
                    <a href="/privacy" className="text-primary hover:underline" target="_blank">{t('privacyPolicy')}</a>.
                  </span>
                </label>
              )}

              <Button type="submit" disabled={loading || (mode === 'signup' && !ageConsent)} className="w-full h-12 rounded-xl font-semibold">
                {loading ? t('oneMoment') : mode === 'login' ? t('logIn') : t('createAccount')}
              </Button>
            </form>
