...
import { DollarSign, Eye, CheckCircle, Music4, ArrowRight, Users, Search } from 'lucide-react';
              ...
              <motion.div key={c.id} initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:i*0.06,duration:0.4}} whileHover={{y:-2}} className="rounded-2xl bg-white/[0.03] backdrop-blur-xl border border-white/[0.06] overflow-hidden flex flex-col">
                ...
                <div className="p-5 flex flex-col flex-1 space-y-4">
                  <div>
                    ...
                    <div className="flex gap-1 flex-wrap mt-1">{(c.genres||'Various').split(',').slice(0,3).map(g=><Badge key={g} variant="secondary" className="text-[10px]">{g.trim()}</Badge>)}</div>
                  </div>
                  ...
                  <div className="mt-auto flex items-center justify-between">
                    ...
                    <Link href={`/creators/${c.id}`}>
                      <Button variant="outline" size="sm" className="text-xs group">
                        View profile <ArrowRight size={12} className="ml-1 transition-transform group-hover:translate-x-0.5"/>
                      </Button>
                    </Link>
                  </div>
                </div>
