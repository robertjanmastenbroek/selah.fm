        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-6">
            ...
            <div className="lg:sticky lg:top-24 space-y-6">
              ...
              {/* Streaming links — detect platform from URL */}
              {tracks.slice(0, 4).map((track: any) => {
                const url = track.spotify_url || track.track_url;
                if (!url) return null;
                const isSpotify = url.includes('open.spotify.com') || url.includes('spotify.com');
                const isAppleMusic = url.includes('music.apple.com') || url.includes('apple.com/music');
                const isYoutube = url.includes('youtube.com') || url.includes('youtu.be');
                const isBandcamp = url.includes('bandcamp.com');
                const isSoundCloud = url.includes('soundcloud.com');
                const isDeezer = url.includes('deezer.com');
                const isTidal = url.includes('tidal.com');

                let icon, label, bgColor, borderColor, textColor;
                if (isSpotify) {
                  icon = <svg width="20" height="20" viewBox="0 0 24 24" fill="#1DB954"><path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381 4.26-1.26 11.28-1.02 15.721 1.621.539.3.719 1.02.419 1.56-.299.421-1.02.599-1.559.3z"/></svg>;
                  label = 'Listen on Spotify'; bgColor = 'bg-green-500/5'; borderColor = 'border-green-500/10'; textColor = 'text-green-400';
                } else if (isAppleMusic) {
                  icon = <span className="text-lg">🍎</span>;
                  label = 'Listen on Apple Music'; bgColor = 'bg-red-500/5'; borderColor = 'border-red-500/10'; textColor = 'text-red-400';
                } else if (isYoutube) {
                  icon = <span className="text-base">▶️</span>;
                  label = 'Watch on YouTube'; bgColor = 'bg-red-500/5'; borderColor = 'border-red-500/10'; textColor = 'text-red-400';
                } else if (isBandcamp) {
                  icon = <span className="text-lg">🎵</span>;
                  label = 'Listen on Bandcamp'; bgColor = 'bg-cyan-500/5'; borderColor = 'border-cyan-500/10'; textColor = 'text-cyan-400';
                } else if (isSoundCloud) {
                  icon = <span className="text-lg">☁️</span>;
                  label = 'Listen on SoundCloud'; bgColor = 'bg-orange-500/5'; borderColor = 'border-orange-500/10'; textColor = 'text-orange-400';
                } else if (isDeezer) {
                  icon = <span className="text-base">🎧</span>;
                  label = 'Listen on Deezer'; bgColor = 'bg-blue-500/5'; borderColor = 'border-blue-500/10'; textColor = 'text-blue-400';
                } else if (isTidal) {
                  icon = <span className="text-base">🌊</span>;
                  label = 'Listen on Tidal'; bgColor = 'bg-blue-500/5'; borderColor = 'border-blue-500/10'; textColor = 'text-blue-400';
                } else {
                  let hostname = 'music service';
                  try { hostname = new URL(url).hostname.replace('www.', ''); } catch {}
                  icon = <span className="text-base">🔗</span>;
                  label = `Listen on ${hostname}`;
                  bgColor = 'bg-white/5'; borderColor = 'border-white/10'; textColor = 'text-muted-foreground';
                }

                const hoverBg = bgColor.replace('bg-', 'hover:bg-').replace('/5', '/10');

                return (
                  <a key={track.id} href={url} target="_blank" rel="noopener noreferrer"
                    className={`flex items-center gap-3 p-3 rounded-xl ${bgColor} border ${borderColor} ${hoverBg} transition-all`}>
                    <span className="shrink-0">{icon}</span>
                    <span className={`text-xs font-medium ${textColor}`}>{label}</span>
                    {track.track_title && <span className="text-[9px] text-muted-foreground/50 ml-auto truncate max-w-[100px]">{track.track_title}</span>}
                  </a>
                );
              })}
